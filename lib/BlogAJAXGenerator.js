...
class BlogAJAXGenerator
{
        ...
        return ( async() =>
        {
            ...
            if( source_markdowns.length > 0 && config.replaced_text )
            {
                ...
                return source_markdowns.map( md_text =>
                    template.replace( config.replaced_text, render( md_text ) )
                );
            }
            return [];
        })();
    ...
    async read_directory()
    {
        ...
        try
        {
            const files = await request( this.config.source_directory );
            const is_markdown = item => item.name.match(/.md$/g);
            this.directory_files = files.filter( is_markdown ).map( item => item.download_url );
        }
        catch ( err )
        {
    ...
    async main( config = {
        "mode": "ajax",
        "source_directory": "https://api.github.com/repos/iigmir/blog-source/contents/articles",
