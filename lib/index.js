...
class StaticSiteGenerator
{
    ...
    async read_directory()
    {
        const handler = async({ mode = "local", directory = "" }) =>
        {
            ...
            {
            case "local":
                // { source_directory = "", directory_files = [] }
                return this.fs_handler.read_source_directory( directory );
            case "ajax":
                return this.ajax_handler.read_source_directory( directory );
            default:
                console.error( mode );
                error_handling( new Error( "Unsupport type" ));
                return [];
            }
        };
        try
