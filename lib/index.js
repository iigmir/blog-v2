...
class FileSystemHandler
{
     ...
     * @param resolve {String} - Promise resolve API.
     * @param reject {String} - Promise reject API.
     * @return {Object} - Config object if success.
     */
    read_config_file({ config_path = "", resolve, reject, })
    {
        return fs.readFile( config_path, "utf8", ( fs_error, file ) => 
        {
    ...
    read_source_markdowns( file_path = "" )
    {
    ...
    read_local_source_directory({ source_directory = "", resolve, reject })
    {
        if( source_directory )
        {
...
class StaticSiteGenerator 
{
    ...
    constructor()
    {
        this.fs_handler = new FileSystemHandler();
    }
    /**
    ...
    get source_markdowns()
    {
        ...
        if( this.directory_files.length > 0 && mode === "local" )
        {
            const read_file = source_file =>
            {
                ...
                return this.fs_handler.read_source_markdowns( fpath );
            };
    ...
    async read_directory()
    {
        ...
        {
            this.fs_handler.read_local_source_directory({ source_directory, resolve, reject });
        };
