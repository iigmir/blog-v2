...
class StaticSiteGenerator
{
     ...
     * @return {("local"|"ajax"|"custom"|"unknown")} Four modes.
     */
    get mode()
    {
    ...
    async do_custom_scriipt()
    {
        const file_path = this.config.custom_script;
        if( file_path )
        {
            const child = execFile( "node", [ file_path ], (error, stdout, stderr) =>
            {
